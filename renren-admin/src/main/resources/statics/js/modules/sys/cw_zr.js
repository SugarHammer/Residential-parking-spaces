$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            { label: '被转让住户编号', name: 'bzrNum', index: 'bzrNum', width: 80 },
            { label: '被转让用户姓名', name: 'bzrUsername', index: 'bzrUsername', width: 80 },
            { label: '被转让手机号', name: 'bzrPhone', index: 'bzrPhone', width: 80 },
        ],
        viewrecords: true,
        height: 385,
        rowNum: 10,
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            alert(vm.cwZr.id);
            var url = vm.cwZr.id == null ? "sys/cwZr/save" : "sys/cwZr/updateZr";
            $.ajax({
                type: "POST",
                url: baseURL + url,
