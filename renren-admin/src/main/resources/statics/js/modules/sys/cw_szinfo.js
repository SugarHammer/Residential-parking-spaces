$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/cwSell/list3',
        datatype: "json",
        colModel: [
            { label: '车位id', name: 'id', index: 'id', width: 40 },
            { label: '车主手机号', name: 'phone', index: 'phone', width: 80 },
            { label: '车位编号', name: 'cwNum', index: 'cwNum', width: 80 },
            { label: '时租价格', name: 'zlPrice', index: 'zlPrice', width: 80 },
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            var url = vm.cwSell.id == null ? "sys/cwSell/saveSzLease" : "sys/cwSell/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
